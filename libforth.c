...
static int ogetwrd(forth *o, uint8_t *p) { assert(o && p);
        int n = 0;
        if(o->stringin) {
...
static int compile(forth *o, mw code, char *str) { assert(o && code < LAST);
        mw *m = o->m;
...
int forth_coredump(forth *o, FILE *dump) { 
        return !o || !dump 
                  || sizeof(*o) != fwrite(o, 1, sizeof(*o), dump) ? -1: 0; 
}
...
int forth_run(forth *o) { assert(o);
        mw *m, pc, *S, I, f = 0, w;
        assert(o && !o->invalid);
